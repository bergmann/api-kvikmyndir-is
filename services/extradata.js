...
var inserFilmsToServices = function(
  ...
  if (collectionName === 'upcoming') {
    console.log('Inserting', films.length, 'upcoming movies to database');
  }
  DBService.insertDocument(films, collectionName, function(err, result) {
    if (err) {
      ...
      if (collectionName === 'upcoming') {
        console.log('Error inserting upcoming movies:', err);
      }
    } else {
      if (collectionName === 'upcoming') {
        console.log('Successfully inserted upcoming movies to database');
      }
    }
  });
