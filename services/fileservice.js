...
var fileservice = {
  ...
  writeToJson: function(data, path, callback) {
    // Write with UTF-8 encoding and proper formatting (2 space indentation)
    fs.writeJson(path, data, { spaces: 2, encoding: 'utf8' }, function(err) {
      if (err) {
      ...
      if (movies[i].poster && movies[i].title) {
        (function(poster, title, i) {
          var posterPath =
            ...
            path.extname(poster);
          promises.push(
            HttpService.getContentBinary(poster)
              .then(function(data) {
                return fileservice.writeImgToFS(data, posterPath, function() {
                  var s = posterPath;
                  movies[i].posterLocal.normal = s.substring(
                    s.indexOf('/posters'),
                    s.length
                  );
                });
              ...
              .catch(function(error) {
                logger
                  .error()
                  .info('Error requesting file from url : ' + error);
              })
          );
