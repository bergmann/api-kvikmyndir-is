  ...
  router
    ...
    .post("/", isLoggedIn, function (req, res, next) {
      ...
      if (!hasError) {
        ...
        req.flash(
          "msg",
          "<h2>Thank you</h2><p>User " + user.username + " created.</p>"
        );
        ...
        DBService.insertAny(user, "users", function (err, document) {
          if (err) {
            ...
            if (user) {
              var userHeader =
                "<h1>" +
                document.ops[0].username +
                " was created in database</h1>";
              var userTable = utils.createHtmlTableFromObjectProperty(
                document.ops[0]
              );
              mailService.sendMail(
                "sindriber@gmail.com",
                "sindriber@gmail.com",
                document.ops[0].fullname +
                  " created in users collection in database",
                "",
