    ...
    router
        ...
        .post("/", function (req, res, next) {
            ...
            for (var key in req.body) {
                var keyName = key
                    .substring(key.indexOf("_") + 1, key.length)
                    .toLowerCase();
                ...
                if (key.toLowerCase().indexOf("email") > -1) {
                    ...
                    _underscore.isEmpty(req.body[key]) &&
                    key !== "process" &&
                    key !== "contactform"
                ) {
                ...
                // Add to user object
                if (
                    keyName.indexOf("username") > -1 ||
                    keyName.indexOf("password") > -1 ||
                    keyName.indexOf("name") > -1 ||
                    keyName.indexOf("message") > -1 ||
                    keyName.indexOf("domain") > -1
                ) {
          ...
          req.flash(
            "msg",
            "<h2>Thank you</h2><p>I will be in touch as soon as possible</p>"
          );
        ...
        // API key form
                ...
                req.flash(
                    "msg",
                    `<h2>Thank you</h2><p>${user.username} account has been activated. Please use it responsibly.</p>`
                );
                ...
                DBService.insertAny(user, "users", function (err, document) {
                    if (err) {
                        logger
                            .databaseError()
                            .info(
                                `Error creating user ${user.username}, Error: ${err}`
                            );
                    } else {
                        if (user) {
                            var userHeader = `<h1>${document.ops[0].username} was created in database</h1>`;
                            var userTable =
                                utils.createHtmlTableFromObjectProperty(
                                    document.ops[0]
                                );
                            mailService.sendMail(
                                emailConfig.email,
                                emailConfig.email,
                                `Application for user ${document.ops[0].fullname}`,
                                "",
                                userHeader + userTable
                            );
                        }
                    }
                });
        ...
        .get("/movies", tokenAuthentication, function (req, res, next) {
            queryMovies(
                ...
                next,
                "movies0",
                config.dataBasePath + "/movies0.json"
            );
        })
        ...
        .get("/theaters", tokenAuthentication, function (req, res, next) {
            queryMovies(req, res, next, "theaters", config.theatersfilepath);
        })
...
var queryMovies = function (req, res, next, collection, filePath) {
    ...
    // Query from DB after imdbid
        ...
        DBService.findDocument(
            {
                $or: [
                    {
                        "ids.imdb": new RegExp(
                            imdbidNumberOnly ? imdbidNumberOnly : imdbid
                        ),
                    },
                    { "omdb.imdbID": imdbid },
                ],
        ...
        DBService.findDocuments(
            {
                showtimes: {
                    $elemMatch: {
                        schedule: new RegExp(req.query.showtime, "i"),
                    },
                },
            },
            collection,
    ...
    // Query from DB after certificate
        ...
        DBService.findDocuments(
            {
                "certificateExtra.number": new RegExp(
    ...
    // Query from DB after actors
        ...
        DBService.findDocuments(
            { "actors_abridged.name": new RegExp(req.query.actor, "i") },
            collection,
            function (err, docs) {
                if (err) {
                    renderResponse(
    ...
    // Query from DB after director
        ...
        DBService.findDocuments(
            { "directors_abridged.name": new RegExp(req.query.director, "i") },
            collection,
            function (err, docs) {
                if (err) {
                    renderResponse(
    ...
    // Query from DB after mongodb uniq id
        ...
        //if (ObjectId.isValid(req.query.mongoid)) {
        var o_id = new ObjectId(req.query.mongoid);
        ...
        if (err) {
            ...
            if (_underscore._.isEmpty(query)) {
                ...
                // Filter imdb id
                    ...
                    if (imdbid.substring(0, 2) === "tt") {
                        var imdbidNumberOnly = imdbid.substring(
                            2,
                            imdbid.length
                        );
                    }
                ...
                // than query number
                    ...
                    data.filter(function (movie) {
                        if (movie.omdb && movie.omdb[0]) {
                            var movieRating = movie.omdb[0].imdbRating
                                ? parseFloat(movie.omdb[0].imdbRating)
                                : null;
                            if (movieRating >= number) {
                                filteredArray.push(movie);
                            }
                        } else if (movie.ratings) {
                            var movieRating = movie.ratings.imdb
                ...
                // Filter certificate
                    ...
                    data.filter(function (movie) {
                        if (
                            movie.certificateExtra &&
                            movie.certificateExtra.number &&
                            movie.certificateExtra.number.toLowerCase() ===
                                query.certificate.toLowerCase()
                        ) {
                            filteredArray.push(movie);
                        }
                    });
                        ...
                        if (movie.actors_abridged) {
                            movie.actors_abridged.forEach(function (actor) {
                                if (
                                    actor.name
                                        .toLowerCase()
                                        .indexOf(query.actor.toLowerCase()) > -1
                                ) {
                                    match = true;
                                }
                            });
                ...
                // Filter director
                    ...
                    data.filter(function (movie) {
                        if (movie.directors_abridged) {
                            movie.directors_abridged.forEach(function (
                                director
                            ) {
                                if (
                                    director.name
                                        .toLowerCase()
                                        .indexOf(query.director.toLowerCase()) >
                                    -1
                                ) {
                                    match = true;
                                }
                ...
                // Let know that mongo id is not possible
                    ...
                    filteredArray.push({
                        ...
                        Message:
                            "Mongoid is not filterable from filesystem. It's only filterable from database",
                    });
                }
...
function renderResponse(req, res, err, docs, queryparam, callback, filePath) {
    ...
    // Debug logging for upcoming endpoint
    if (filePath && filePath.includes('upcoming')) {
        console.log('Upcoming renderResponse - err:', err, 'docs count:', docs ? docs.length : 0);
    }

    // If db returns error, read from json file instead
    if (err || (docs && docs.length === 0)) {
        if (filePath && filePath.includes('upcoming')) {
            console.log('Upcoming: Falling back to JSON file');
        }
        if (!callback) {
            res.send({
        ...
        if (filePath && filePath.includes('upcoming')) {
            console.log('Upcoming: Returning from MongoDB');
        }
        res.json(docs);
    }
}
